...
DELIMITER $$
USE `mydb`$$
CREATE PROCEDURE populate_eval_for()
BEGIN
    -- Inserting data into Eval_for from Assignment joined with Judges
    INSERT INTO Eval_for (
        Assignement_Episode_episode_number, 
        Assignement_National_Cuisine_name, 
        Assignement_Recipies_name, 
        Assignement_Cook_name, 
        Assignement_Cook_surname, 
        Judge_Cook_name, 
        Judge_Cook_surname, 
        evaluation
    )
    SELECT 
        ...
        J.Cook_name, 
        J.Cook_surname,
        FLOOR(1 + RAND() * 5) -- Random evaluation between 1 and 5
    FROM 
        Assignment A
    INNER JOIN 
        Judges J ON A.Episode_episode_number = J.Episode_episode_number;
END;$$
