...
CREATE TABLE IF NOT EXISTS `mydb`.`Recipies_has_Thematic Unit` (
  `Recipies_name` VARCHAR(50) NOT NULL,
  `Thematic Unit_name` VARCHAR(50) NOT NULL,
...
BEGIN
    ...
    IF episode_count <= 3 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Constraint violation: There cannot be less than 3 judges per episode.';
    END IF;
END$$

USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`asssignment_BEFORE_INSERT` BEFORE INSERT ON `asssignment` FOR EACH ROW
BEGIN
	DECLARE episode_count INT;
...
USE `mydb`$$
CREATE DEFINER = CURRENT_USER TRIGGER `mydb`.`asssignment_BEFORE_DELETE` BEFORE DELETE ON `asssignment` FOR EACH ROW
BEGIN
	DECLARE episode_count INT;
